...
public class EaseChatFragment extends BaseFragment implements EMMessageListener {
    ...
    protected void baseInit() {
        ...
        EaseUserUtils.toChatId = toChatId;
...
//        });
                ...
                public void run() {
                    try {
                        EMClient.getInstance().chatManager().fetchHistoryMessages(
                                toChatId, EaseCommonUtils.getConversationType(chatType), pagesize, "");
                        final List<EMMessage> msgs = conversation.getAllMessages();
                        int msgCount = msgs != null ? msgs.size() : 0;
                        if (msgCount < conversation.getAllMsgCount() && msgCount < pagesize) {
                            String msgId = null;
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            if (null != data) {
                if (requestCode == REQUEST_CODE_LOCAL) { // send local image
                    ...
                    EMMessage dingMsg = EaseDingMessageHelper.get().createDingMessage(toChatId, msgContent);
                    sendMessage(dingMsg);
                }
            }
        }
        ...
        public void onClick(int itemId, View view) {
            ...
            switch (itemId) {
                ...
                case ITEM_PICTURE:
                    ...
                    break;
                case ITEM_LOCATION:
                    JumpUtils.jump(EaseChatFragment.this, EaseBaiduMapActivity.class, null, REQUEST_CODE_MAP);
                    break;
            }
        }
    ...
    protected void sendMessage(EMMessage message) {
        ...
        message.setMessageStatusCallback(messageStatusCallback);
        String imgUrl = ApiService.BASE_PIC_URL + SpfUtils.getSpfSaveStr(DocConstant.DocPhotoUrl);
        message.setAttribute("hxChartHeadImg", imgUrl);
    ...
    protected void selectPicFromCamera() {
        if (!EaseCommonUtils.isSdcardExist()) {
            T.showLong(R.string.sd_card_does_not_exist);
            return;
        }
        ...
        Intent takePictureIntent = PhotoUtils.takePicture(getActivity(), cameraFile);
...
//    接收消息 接口 start
    ...
    public void onMessageReceived(List<EMMessage> messages) {
        for (EMMessage message : messages) {
            ...
            if (username.equals(toChatId) || message.getTo().equals(toChatId) || message.conversationId().equals(toChatId)) {
                messageList.refreshSelectLast();
