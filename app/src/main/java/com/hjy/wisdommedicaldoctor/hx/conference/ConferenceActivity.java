package com.hjy.wisdommedicaldoctor.hx.conference;
...
import com.hjy.wisdommedicaldoctor.hx.widget.VMViewGroup;
...
public class ConferenceActivity extends AppCompatActivity implements EMConferenceListener {
    ...
    VMViewGroup callConferenceViewGroup;
    ...
    private void miniMode() {
        Toast.makeText(this, "暂未实现最小化", Toast.LENGTH_SHORT).show();
    }
        ...
        EMClient.getInstance()
                ...
                .createAndJoinConference(password, new EMValueCallBack<EMConference>() {
                    ...
                    public void onSuccess(EMConference value) {
                        Log.e("", "create and join conference success");
                        conference = value;
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(ConferenceActivity.this, "Create and join conference success",
                    ...
                    public void onError(int error, String errorMsg) {
                        Log.e("Create and join failed", errorMsg);
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(ConferenceActivity.this, "Create and join conference error",
                                               Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                ...
                .joinConference(confId, password, new EMValueCallBack<EMConference>() {
                    ...
                    public void onSuccess(EMConference value) {
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(ConferenceActivity.this, "Join conference success",
                                               Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                    ...
                    @Override
                    public void onError(int error, String errorMsg) {
                        Log.e("Join failed %d, %s", errorMsg);
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(ConferenceActivity.this, "Join conference success",
        ...
        alertDialogBuilder.setPositiveButton("确认", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                if (TextUtils.isEmpty(editText.getText().toString().trim())) {
                    Toast.makeText(ConferenceActivity.this, "输入不能为空", Toast.LENGTH_LONG).show();
                    return;
                }
                        ...
                        .inviteUserToJoinConference(conference.getConferenceId(),
                                                    ...
                                                    new EMValueCallBack() {
                                                        ...
                                                        public void onSuccess(Object value) {
                                                            Log.e("", "invite join conference success");
                                                        }

                                                        @Override
                                                        public void onError(int error,
                                                                String errorMsg) {
                                                            Log.e("invite join failed", errorMsg);
                                                        }
                                                    });
            }
        });
    ...
    private void exitConference() {
        ...
        EMClient.getInstance().conferenceManager().exitConference(new EMValueCallBack() {
            ...
            @Override
            public void onError(int error, String errorMsg) {
                Log.e("exit conference failed", errorMsg);
                finish();
            }
        });
    }
    ...
    private void publish() {
        EMClient.getInstance()
                ...
                .publish(normalParam, new EMValueCallBack<String>() {
                    ...
                    @Override
                    public void onError(int error, String errorMsg) {
                        Log.e("publish failed %d, %s", errorMsg);
                    }
                });
    }
    ...
    public void publishDesktop() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            desktopParam.setShareView(this.getWindow().getDecorView());
        }
        EMClient.getInstance()
                .conferenceManager()
    ...
    private void unpublish(String publishId) {
        ...
        EMClient.getInstance()
                ...
                .unpublish(publishId, new EMValueCallBack<String>() {
                    ...
                    @Override
                    public void onError(int error, String errorMsg) {
                        Log.e("unpublish failed %d, %s", errorMsg);
                    }
                });
    }
    ...
    private void subscribe(EMConferenceStream stream, final ConferenceMemberView memberView) {
        EMClient.getInstance()
                ...
                .subscribe(stream, memberView.getSurfaceView(), new EMValueCallBack<String>() {
                    ...
                    @Override
                    public void onError(int error, String errorMsg) {
                        Log.e("subscribe failed %d, %s", errorMsg);
                    }
                });
    }
    ...
    public void onMemberJoined(final String username) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(ConferenceActivity.this, username + " joined conference!", Toast.LENGTH_SHORT)
                     .show();
            }
        });
    }
    ...
    public void onMemberExited(final String username) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(ConferenceActivity.this, username + " removed conference!", Toast.LENGTH_SHORT)
                     .show();
            }
        });
    }
    ...
    public void onStreamAdded(final EMConferenceStream stream) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(ConferenceActivity.this, stream.getUsername() + " stream add!", Toast.LENGTH_SHORT)
                     .show();
    ...
    public void onStreamRemoved(final EMConferenceStream stream) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(ConferenceActivity.this, stream.getUsername() + " stream removed!",
                               Toast.LENGTH_SHORT).show();
    ...
    public void onStreamUpdate(final EMConferenceStream stream) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(ConferenceActivity.this, stream.getUsername() + " stream update!",
                               Toast.LENGTH_SHORT).show();
    ...
    public void onPassiveLeave(final int error, final String message) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(ConferenceActivity.this, "Passive exit " + error + ", message" + message,
                               Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    public void onConferenceState(final ConferenceState state) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(ConferenceActivity.this, "State=" + state, Toast.LENGTH_SHORT).show();
            }
        });
    }
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                if (streamId.equals(conference.getPubStreamId(EMConferenceStream.StreamType.NORMAL))
                        || streamId.equals(
                        conference.getPubStreamId(EMConferenceStream.StreamType.DESKTOP))) {
                    Toast.makeText(ConferenceActivity.this, "Publish setup streamId=" + streamId,
                                   Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(ConferenceActivity.this, "Subscribe setup streamId=" + streamId,
                                   Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    public void onReceiveInvite(final String confId, String password, String extension) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(ConferenceActivity.this, "Receive invite " + confId, Toast.LENGTH_LONG).show();
            }
        });
    }
}
