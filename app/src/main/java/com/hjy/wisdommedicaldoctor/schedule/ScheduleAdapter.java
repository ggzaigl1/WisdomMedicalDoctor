...
/**
 * 日程列表 适配器
 * Created by fangs on 2018/7/27 17:33.
 */
public class ScheduleAdapter extends BaseQuickAdapter<TodayBean.ListConsultRecordBean, BaseViewHolder> {

    ScheduleAdapter(@Nullable List<TodayBean.ListConsultRecordBean> data) {
...
//                tv_appointment_time.setVisibility(View.VISIBLE);
        ...
        if (!TextUtils.isEmpty(item.getReserveTime()) && !TextUtils.isEmpty(item.getReserveDate())) {
